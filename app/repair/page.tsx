...
export default function Repair() {
  ...
  return (
    <main className="flex-1 flex items-center justify-center px-4 py-16 bg-neutral-50 dark:bg-[#0a0a0a]">
      <div className="w-full max-w-md">

        <div className="mb-8">
          <p className="text-[#2BD9C6] text-xs font-black uppercase tracking-[0.3em] mb-3">Book a repair</p>
          <h1 className="text-4xl font-black uppercase tracking-tighter text-neutral-900 dark:text-white">Repair Request</h1>
          <p className="text-neutral-500 dark:text-white/40 text-sm mt-2">Fill this out and we&apos;ll get back to you with a quote.</p>
        </div>
        ...
        {submitted ? (
          <div className="border border-[#2BD9C6]/30 bg-[#2BD9C6]/5 rounded-lg p-8 text-center">
            ...
            <p className="text-neutral-900 dark:text-white font-black uppercase text-lg tracking-tight mb-2">Request Sent</p>
            <p className="text-neutral-500 dark:text-white/50 text-sm mb-6">We&apos;ll be in touch soon with a quote.</p>
            <button onClick={handleReset} className="px-6 py-2.5 bg-[#2BD9C6] text-black font-black uppercase tracking-wide text-sm rounded hover:bg-[#1fc4b3] transition">
              Submit another
            </button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <textarea
              ...
              placeholder="Describe the damage — board type, size, where the ding is"
              required
            ...
            <label className="cursor-pointer block border border-neutral-200 dark:border-white/10 text-neutral-400 dark:text-white/60 text-sm text-center py-3 rounded hover:border-[#2BD9C6]/60 hover:text-neutral-700 dark:hover:text-white transition">
              {fileName ? `📎 ${fileName}` : 'Attach a photo (optional)'}
              <input
                ref={fileRef}
                type="file"
                accept="image/*"
        ...
        <p className="text-neutral-400 dark:text-white/20 text-xs text-center mt-6">
          View our{' '}
          <Link href="/services" className="underline underline-offset-2 hover:text-neutral-600 dark:hover:text-white/50 transition">
            pricing guide
          </Link>
          {' '}before submitting
        </p>
